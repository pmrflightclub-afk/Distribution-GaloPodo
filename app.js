...
const CHANGELOG = [
  ...
  },
  {
    ...
    ajouts: [
      'Planche — rognage par image : en mode « Éditer les images », activez « ✂️ Rogner des images », touchez les images à rogner (cadre rouge), puis « Valider ». Seules les images choisies sont agrandies (le rognage n\'est plus global).',
    ],
    corrections: [
      'Boutons inactifs (ex. RDV / Paiement avant le jour de la tournée) : ils apparaissent désormais grisés pour montrer qu\'ils ne sont pas encore disponibles.',
    ],
  },
  {
  ...
  },
  {
    ...
    ajouts: [
      'Planches — cadrage des images : nouveau bouton « ✏️ Éditer les images » au-dessus de la grille. En mode édition, un cadrillage d\'aide s\'affiche sur chaque case (marges + axe central rouge) et toucher une image ouvre un éditeur pour la zoomer, la déplacer et la redresser (rotation). Le cadrage est repris à l\'identique sur le PDF (rogné à la case) ; le cadrillage, lui, n\'apparaît jamais sur le PDF. En mode normal, toucher une image la retire (comme avant).',
      'Gestion → Planche : réglage des marges du cadrillage d\'aide (gauche/droite et haut/bas, en cm) — repère visuel pour aligner le haut du pied et le centre.',
    ],
  },
  {
  ...
  },
  {
    ...
    ajouts: [
      'Photos / planches facturées : Gestion → Articles → « Photos » — 3 tarifs (3, 4, 5 angles) avec TTC automatique et prix par photo. Un interrupteur calcule les tarifs 4 & 5 angles en pourcentage à partir du prix/photo de la référence (3 angles), sinon ils sont manuels.',
      'Acte « Photo » dans un arrêt : cocher Photo ouvre une fenêtre pour choisir le tarif (nombre d\'angles) ET le(s) type(s) de planche — parage, ferrage, déferrage, radio (plusieurs possibles par cheval). Chaque type facturable ajoute son tarif à la facture (une ligne « Photos / planches » consolidée par client). La radio est suivie mais jamais facturée.',
      'Statut des planches par TYPE : le badge 📷 (rouge/orange/vert) et le « Compte rendu photo » tiennent compte de chaque type. Faire une planche d\'un type la marque « faite » pour ce type précis.',
      'Création de planche (Déclarer) : les dates proposées pour un cheval montrent la date ET le type ; cliquer l\'une remplit automatiquement la date et le type de la planche.',
    ],
  },
  {
  ...
  },
  {
    ...
    ajouts: [
      'Badge « ✓ prête » des tournées : signifie maintenant que le PARAMÉTRAGE est complet (adresses localisées, itinéraire calculé, au moins un cheval présent par client, heure de RDV par client) — plus besoin d\'avoir coché les actes ou encaissé. Sinon « ⚠ à compléter », et « 🔄 recalculer itinéraire » en priorité si l\'ordre des arrêts a changé.',
      'Éditeur de tournée : nouvelle section « 🏁 Retour » en bas (trajet de retour estimé + bouton « Route retour » pour le temps réel), comme dans le Trajet du jour.',
      'Prise de RDV : au moins un cheval (avec date et heure) est désormais obligatoire pour enregistrer un rendez-vous (idem pour le 2ᵉ RDV de suivi pathologique).',
    ],
    corrections: [
      'Les heures de RDV par client ne disparaissent plus à la réouverture de la tournée (la resynchronisation des chevaux/adresses les conservait mal).',
      'Bouton « 🧭 Étape » : ouvre bien l\'itinéraire de l\'arrêt PRÉCÉDENT vers l\'arrêt courant (au lieu de courant → suivant).',
      'Itinéraire : quand on change l\'ordre des arrêts, qu\'on en ajoute/retire un ou qu\'une adresse change, les temps de trajet réels déjà encodés passent en « 🔄 à recalculer » (boutons Route en orange, Retour en rouge) et ne faussent plus les statistiques de temps de travail.',
      'Tournée « à venir » : les boutons 💶 Paiement et 📅 RDV sont grisés tant que le jour de la tournée n\'est pas arrivé (actifs le jour J, verts une fois faits).',
    ],
  },
  {
  ...
  },
  {
    ...
    ajouts: [
      'Tournée : la case « 2ᵉ RDV — suivi rapproché » est déplacée dans l\'en-tête du client (entre RDV/Paiement et Réduction), avec une case PAR CHEVAL concerné — elle n\'apparaît que si fourbure / NPAS / infection est coché pour ce cheval.',
      'Listes de tournées (Accueil + Tournées) : la date de la tournée s\'affiche dans la couleur de l\'app ; la mention « À venir » est retirée du titre (redondante avec la section) ; une tournée clôturée affiche en rouge « il y a N j / N sem ».',
      'Programmer le suivi (RDV) : les options « ne pas replacer » et « date différente » sont empilées l\'une au-dessus de l\'autre ; la section « suivi pathologique » est indentée sous le cheval ; l\'alerte « déjà un RDV » ignore la tournée ouverte et ne regarde qu\'à partir du lendemain de sa date.',
    ],
  },
  {
  ...
  },
  {
    ...
    ajouts: [
      'Tournée — le bouton « Annuler / reporter » d\'un cheval est maintenant un vrai bouton visible (⊘ Annuler / ✎ Gérer), plus une petite annotation.',
      'Annulation d\'un cheval : le paiement du client doit désormais être validé AVANT d\'annuler (l\'app propose d\'ouvrir le paiement si besoin) — une note de crédit est alors créée.',
      'Report d\'un cheval : aucun paiement à valider, mais un champ « Montant TTC à facturer à la prochaine visite » (facultatif). Le montant est mémorisé et ajouté automatiquement à la prochaine tournée du client.',
    ],
  },
  {
  ...
  },
  {
    ...
    ajouts: [
      'Gestion → Clients : la liste affiche désormais 📍 le nom des adresses spécifiques des chevaux (celles qui diffèrent de l\'adresse du client ou de la société).',
      'Gestion → Adresses chevaux : chaque adresse indique sa source (client / société / spécifique). Pour une adresse spécifique, son nom (ex. « Écurie du Nord ») est modifiable directement dans la liste et met à jour la fiche du cheval ; un nom vide peut être encodé sur place. Un client peut avoir plusieurs adresses de chevaux.',
    ],
  },
  {
  ...
  },
  {
    ...
    ajouts: [
      'Frais véhicule — refonte du cycle de vie : on ne « renouvelle » plus un poste sur place, on déclare le remplacement et un nouveau poste prend le relais (l\'ancien passe en historique). Les boutons « Fait » / « Refaire ce type » ont été retirés.',
      'Chaque élément lié (Disques, Montage, Pièces…) a désormais sa propre échéance (son propre kilométrage prévu) et peut échoir indépendamment de son type.',
      'Nouvelle « 🧾 Note de frais véhicule » (Déclarer un événement, et page Statut véhicule) : indiquez le montant réel, puis si la dépense remplace un poste suivi (avec le sort de chaque élément lié : maintenu / changé aussi / abandonné), crée un nouveau poste à suivre, ou est une simple dépense ponctuelle. Le montant réel alimente la stat « provision vs réel ».',
      'Contrôle véhicule (fusionné dans « Statut véhicule », Accueil + Gestion) : les postes/éléments échus s\'affichent avec le dépassement et deux actions — « ✅ Changement fait » (note de frais) et « ⏳ Repousser » (ajoute des km au seuil si le garage n\'a pas encore fait le remplacement, sans changer la provision).',
      'À chaque remplacement, l\'app propose (sans imposer) d\'ajuster l\'intervalle et la provision du nouveau poste au réel observé — pour corriger une prévision trop basse ou trop haute. Historique des remplacements consultable dans Statut véhicule.',
    ],
  },
  {
...
// Un frais provisionne (compte dans la base €/km) tant qu'il n'est pas REMPLACÉ. Un poste échu mais pas encore changé continue de provisionner (on roule encore dessus).
const fraisActif = (f) => f.statut !== 'remplace';
...
function renderClients() {
  ...
  shown.forEach((c) => {
    ...
    const specNoms = Array.from(new Set((c.chevaux || []).filter((h) => (h.addrSource || (h.memeAdresse === false ? 'specifique' : 'client')) === 'specifique').map((h) => chevalAddrNom(c, h)).filter(Boolean))); // adresses spécifiques des chevaux (≠ client/société)
...
// Écart d'une tournée à aujourd'hui : à venir → « J-n / n sem » (vert) ; clôturée/passée → « il y a n j / n sem » (rouge).
function tourEta(date, st) {
  if (!date) return null;
  const p = (s) => { const [y, m, d] = s.split('-').map(Number); return Date.UTC(y, (m || 1) - 1, d || 1); };
  const n = Math.round((p(date) - p(todayStr())) / 86400000);
  if (st === 'avenir' && n > 0) return { text: n < 7 ? 'J-' + n : Math.round(n / 7) + ' sem', cls: 'td-eta' };
...
// ----- Itinéraire : temps réels périmés -----
...
function renderEditorArrets(locked) {
  ...
  const futureTour = statusOf(currentTour) === 'avenir'; // tournée à venir → Paiement & RDV grisés (disponibles le jour J)
  ...
  currentTour.arrets.forEach((a, i) => {
    ...
    const routeStale = realMin === 0; const routeDone = realMin != null && realMin > 0; const hhv = arretHeure(a); const payDone = arretPaiementDone(currentTour, a);
    const routeCls = routeStale ? ' pret-on' : (routeDone ? ' done' : ''); const routeSuffix = routeStale ? ' 🔄' : (routeDone ? ' ✓' : '');
    ...
    nav.innerHTML = `<span class="a-nav-t">🕒 ${estMin != null ? durMin(estMin) + ' est.' : '—'}${routeDone ? ' · <b>' + durMin(realMin) + ' réel</b>' : (routeStale ? ' · <b class="td-stale">🔄 à recalculer</b>' : '')}</span>${locked ? '' : `<span class="a-nav-b"><button class="btn small" data-waze>${navLabel()}</button> <button class="btn small${routeCls}" data-route>Route${routeSuffix}</button>${i > 0 ? ` <button class="btn small${routeCls}" data-etape title="Itinéraire de l'arrêt précédent vers cet arrêt">🧭 Étape${routeSuffix}</button>` : ''}</span>`}`;
    if (!locked) {
      ...
      { const etB = nav.querySelector('[data-etape]'); if (etB) etB.addEventListener('click', () => { const prev = currentTour.arrets[i - 1]; const origAddr = prev ? prev.addr : tourHome(); if (!origAddr || !addrStr(origAddr).trim()) { alert('Pas d\'arrêt précédent ni de départ défini.'); return; } openMapsRoute(addrQuery(origAddr), addrQuery(a.addr)); }); } // étape = arrêt précédent → arrêt courant
    }
    el.appendChild(nav);
    if (!locked) {
    ...
    a.clients.forEach((cl) => {
      ...
      // Cases « 2ᵉ RDV — suivi rapproché » (une PAR CHEVAL en pathologie), affichées dans l'en-tête client entre RDV/Paiement et Réduction.
      const suiviRowsInner = (clObj) => {
        const cli = clients.find((x) => x.id === clObj.clientId);
        const rows = (clObj.chevaux || []).filter((cv) => !chevalCancelled(cv) && (cv.parage || cv.visite) && (cv.fourbure || cv.npas || cv.infection)).map((cv) => {
          const fi = cli ? (cli.chevaux || []).find((h) => norm(h.nom) === norm(cv.nom)) : null;
          if (!fi) return '';
          const s = fi.suiviPatho || null, on = !!(s && s.actif);
          return `<div class="ac-suivi-row"><label class="ch-opt"><input type="checkbox" data-psact="${cv.id}"${on ? ' checked' : ''}/> 🐴 ${esc(cv.nom)}</label>${on ? ` <span class="ch-patho-rec">tous les <input type="number" data-psval="${cv.id}" min="1" max="4" value="${Math.min(4, Math.max(1, s.valeur || 1))}"/> <select data-psunit="${cv.id}"><option value="jours"${s.unite !== 'semaines' ? ' selected' : ''}>jour(s)</option><option value="semaines"${s.unite === 'semaines' ? ' selected' : ''}>semaine(s)</option></select></span>` : ''}</div>`;
        }).filter(Boolean);
        return rows.length ? `<div class="ac-suivi-head">🔁 2ᵉ RDV — suivi rapproché</div>${rows.join('')}` : '';
      };
      // ===== En-tête du client EN TÊTE : nom · TTC · heure du client · Paiement/RDV/Prêt/Planche · réduction =====
      {
        ...
        if (!locked) {
          ...
          actBar.addEventListener('change', (e) => {
            const t2 = e.target; if (!(t2.matches && t2.matches('[data-psact],[data-psval],[data-psunit]'))) return;
            const cli = clients.find((x) => x.id === cl.clientId); const findFi = (cvid) => { const cvo = (cl.chevaux || []).find((x) => x.id === cvid); return (cli && cvo) ? (cli.chevaux || []).find((h) => norm(h.nom) === norm(cvo.nom)) : null; };
            if (t2.matches('[data-psact]')) { const fi = findFi(t2.dataset.psact); if (!fi) return; if (t2.checked) fi.suiviPatho = Object.assign({ unite: 'semaines', valeur: 1 }, fi.suiviPatho || {}, { actif: true }); else if (fi.suiviPatho) fi.suiviPatho.actif = false; saveClients(); const sv = actBar.querySelector('.ac-suivi'); if (sv) sv.innerHTML = suiviRowsInner(cl); }
            else if (t2.matches('[data-psval]')) { const fi = findFi(t2.dataset.psval); if (!fi) return; fi.suiviPatho = Object.assign({ unite: 'semaines', actif: true }, fi.suiviPatho || {}, { valeur: Math.min(4, Math.max(1, parseInt(t2.value, 10) || 1)) }); saveClients(); }
            else if (t2.matches('[data-psunit]')) { const fi = findFi(t2.dataset.psunit); if (!fi) return; fi.suiviPatho = Object.assign({ valeur: 1, actif: true }, fi.suiviPatho || {}, { unite: t2.value }); saveClients(); }
          });
        }
      }
      ...
      if (!locked) {
        ...
        pool.forEach((ph, pi) => {
          ...
          if (cv && !cancelled) {
            ...
            if (photoOn) chips.push('📷 ' + esc((cv.photo.stades || []).join('/')) + ' · ' + (cv.photo.angle || 3) + ' angles');
  ...
  // ----- Section Retour (éditeur) : trajet de retour vers domicile/arrivée + Route retour (temps réel) -----
  if (!locked && currentTour.arrets.length) {
    const retAddr = returnAddrOf(currentTour);
    const R = currentTour.result; const mpk = (R && R.totalKm > 0 && R.totalMin) ? (R.totalMin / R.totalKm) : (60 / (S.vitesseKmh || 90));
    const estRet = (R && R.kmLastHome != null) ? Math.round(R.kmLastHome * mpk) : null;
    const realRet = (typeof currentTour.returnRealMin === 'number') ? currentTour.returnRealMin : null;
    const retStale = realRet === 0; const retDone = realRet != null && realRet > 0;
    const retCls = retStale ? ' danger' : (retDone ? ' done' : ''); const retSuffix = retStale ? ' 🔄' : (retDone ? ' ✓' : '');
    const rr = document.createElement('div'); rr.className = 'arret-retour-row';
    rr.innerHTML = `<div class="a-top"><div class="grow"><b>🏁 Retour</b><div class="li-sub">📍 ${esc(addrStr(retAddr)) || 'domicile'}</div></div></div>
      <div class="a-nav"><span class="a-nav-t">🕒 ${estRet != null ? durMin(estRet) + ' est.' : '—'}${retDone ? ' · <b>' + durMin(realRet) + ' réel</b>' : (retStale ? ' · <b class="td-stale">🔄 à recalculer</b>' : '')}</span><span class="a-nav-b"><button class="btn small" data-rwaze>${navLabel()}</button> <button class="btn small${retCls}" data-rroute>Route retour${retSuffix}</button></span></div>`;
    rr.querySelector('[data-rwaze]').addEventListener('click', () => openNav(retAddr));
    rr.querySelector('[data-rroute]').addEventListener('click', () => modalReturnTime(currentTour, estRet, () => renderEditorArrets(locked)));
    box.appendChild(rr);
  }
...
function photoTariffHT(angle) {
  const P = S.photoTarifs || {}; angle = +angle || 3;
  const base = (P.angle3HT || 0) / PHOTO_NB[3];
  if (angle === 4) return P.pctMode ? base * (1 - (P.pct4 || 0) / 100) * PHOTO_NB[4] : (P.angle4HT || 0);
  if (angle === 5) return P.pctMode ? base * (1 - (P.pct5 || 0) / 100) * PHOTO_NB[5] : (P.angle5HT || 0);
  return P.angle3HT || 0;
}
...
function recapText(R, tour) {
  ...
  if (tour && Array.isArray(tour.arrets) && tour.arrets.length) {
    const withReal = tour.arrets.filter((a) => a.realMin > 0);
...
function renderTrajetTemps() {
  ...
  list.forEach((t) => {
    ...
    const withReal = (t.arrets || []).filter((a) => a.realMin > 0);
    ...
    (t.arrets || []).forEach((a, i) => { const r = (a.realMin > 0) ? durMin(a.realMin) : (a.realMin === 0 ? '<i>🔄 à recalculer</i>' : '<i>non renseigné</i>'); h += `<div class="fin-cheval"><span>${i + 1}. ${esc(labelFor(a)) || 'arrêt'}</span><span>réel : ${r}</span></div>`; });
    el.innerHTML = h; box.appendChild(el);
  });
}
...
function travailForRecovered(t) {
  ...
  const hasRet = t.returnRealMin > 0;
...
function travailForTour(t) {
  ...
  const R = t.result;
  const mpk = (R && R.totalKm > 0 && R.totalMin) ? (R.totalMin / R.totalKm) : (60 / (S.vitesseKmh || 90));
  const travelMin = (i) => { const a = t.arrets[i]; if (a.realMin > 0) return a.realMin; const seg = (R && R.rows && R.rows[i]) ? (R.rows[i].segKm || 0) : 0; return seg * mpk; };
  const returnMinEst = (R && R.kmLastHome != null) ? R.kmLastHome * mpk : 0;
  ...
  const returnMs = (t.returnRealMin > 0) ? t.returnRealMin * 60000 : ((t.endedAt && lastDep) ? Math.max(0, t.endedAt - lastDep) : returnMinEst * 60000);
...
async function planchePageCanvas(pi) {
  ...
  rows.forEach((r, ri) => {
    ...
    angles.forEach((a, ci) => {
      ...
      const key = plCellKey(pi, r, ci), T = st.cellT && st.cellT[key];
...
function fraisJaugeText(f) {
  const nat = f.nature === 'recurrent' ? 'récurrent' : 'exceptionnel';
  if (f.statut === 'remplace') return `✔ remplacé${f.date ? ' le ' + fmtDateFr(f.date) : ''} — hors calcul du tarif`;
  const kd = f.kmDebut || 0, interval = f.kmPrevus || 0, rep = f.kmReport || 0;
  if (!interval) return `${nat} · renseignez l'intervalle (km prévus)`;
  if (!kd) return `${nat} · renseignez le km de départ (achat / dernier remplacement)`;
  const target = kd + interval + rep, echu = odometer() >= target;
  return `${nat} · départ ${km(kd)} → prochain prévu à ${km(target)} (+${km(interval)}${rep ? ' · report +' + km(rep) : ''})${echu ? ' · ⚠ échu' : ''}`;
}
  ...
  S.frais.forEach((f, i) => {
    ...
    el.innerHTML = `<div class="er-top"><span class="drag-h">⠿</span><span class="li-sub" title="Identifiant du frais">#${f.id.slice(-4)}</span>
      ...
      <div class="er-grid">
        ...
        <label>Date de départ<input data-k="date" type="date" value="${f.date || ''}"/></label>
        ...
        <label>Km de départ<input data-k="kmDebut" type="number" step="1000" min="0" value="${f.kmDebut || ''}"/></label>
      ...
      ${children.length ? `<p class="hint">🔗 Éléments liés (échéance propre) : <b>${children.map((c) => esc(c.poste || 'Frais')).join(', ')}</b>.</p>` : ''}
      ${isChild ? '' : `<div class="er-renew"><button class="btn small" data-add-elem>＋ Élément</button></div>`}`;
    ...
    if (kmDebEl) wireNum(kmDebEl, { get: () => f.kmDebut, dec: 0, set: (v) => { f.kmDebut = v; refreshJauge(); }, after: () => saveSettings() });
...
// Gestion → Articles → Photos : 3 tarifs (3/4/5 angles) avec TTC auto + prix/photo. Interrupteur : 4 & 5 en % (base/photo réf. × (1−%) × nb photos).
function renderPhotoTarifs() {
  const box = $('photoTarifsBody'); if (!box) return;
  const P = S.photoTarifs;
  ...
  box.innerHTML = `${row(3)}
    <label class="chk2"><input type="checkbox" id="photoPctMode" ${P.pctMode ? 'checked' : ''}/> Calculer les tarifs 4 &amp; 5 angles en pourcentage (à partir du prix/photo de la référence 3 angles)</label>
    ${row(4)}
    ${row(5)}`;
  const paint = () => {
    [3, 4, 5].forEach((a) => {
...
// Rafraîchit toutes les vues touchées par un changement de frais véhicule.
function refreshFraisUI() { if (typeof renderFraisVehicule === 'function') renderFraisVehicule(); if (typeof renderStatutVehiculePage === 'function') renderStatutVehiculePage(); if (typeof renderProvisionVsReel === 'function') renderProvisionVsReel(); renderHome(); }
// Ajoute une ligne au journal des frais réels (facture d'achat) — pour la stat « provision vs réel ».
function pushFraisJournal(date, kmv, fraisId, poste, montant) { if (!Array.isArray(S.fraisJournal)) S.fraisJournal = []; if ((montant || 0) > 0) S.fraisJournal.push({ id: uid(), date: date || todayStr(), km: Math.max(0, Math.round(kmv || 0)), fraisId: fraisId || null, poste: poste || 'Frais', montant: montant || 0 }); }
// ⏳ Repousser l'échéance d'un frais échu (le garage n'a pas encore changé la pièce) : ajoute des km au seuil, SANS toucher la provision €/km.
function modalRepousser(f) {
  const seuil = fraisSeuil(f), odo = Math.round(odometer());
  openModal(`<div class="modal-head"><b>⏳ Repousser — ${esc(f.poste || 'Frais')}</b><button class="x" id="mX">✕</button></div>
    <p class="hint">Remplacement pas encore fait ? Ajoutez des km au seuil pour repousser l'alerte (la provision €/km n'est pas modifiée). Seuil actuel <b>${km(seuil)}</b> · odomètre estimé <b>${km(odo)}</b>${odo > seuil ? ' · dépassement <b>' + km(odo - seuil) + '</b>' : ''}.</p>
    <label>Km supplémentaires prévus<input type="number" id="rpKm" step="500" min="0" inputmode="numeric" value="2000"/></label>
    <p class="hint" id="rpApercu"></p>
    <div class="actions"><button class="btn primary block" id="rpOk">Repousser l'échéance</button></div>`);
  $('mX').onclick = closeModal;
  const upd = () => { const add = Math.max(0, parseNum($('rpKm').value) || 0); $('rpApercu').innerHTML = `Nouveau seuil : <b>${km(seuil + add)}</b>.`; };
  upd(); $('rpKm').addEventListener('input', upd);
  $('rpOk').onclick = () => { const add = Math.max(0, Math.round(parseNum($('rpKm').value) || 0)); if (add > 0) { f.kmReport = (f.kmReport || 0) + add; saveSettings(); } closeModal(); refreshFraisUI(); };
}
// Remplace un frais : l'ancien devient 'remplace' (hors base véhicule, gardé en historique), un NOUVEAU frais actif prend le relais (km/date/montant réels), chaîné. Renvoie le nouveau. Ne touche PAS au journal (fait par l'appelant, 1 ligne par note).
function replaceFrais(oldF, o) {
  o = o || {};
  const newF = { id: uid(), poste: oldF.poste, nature: oldF.nature, parentId: (o.parentId !== undefined ? o.parentId : (oldF.parentId || null)), statut: 'actif', kmReport: 0,
    kmPrevus: oldF.kmPrevus || 0, montantHT: (o.montantHT != null ? o.montantHT : (oldF.montantHT || 0)),
    kmDebut: Math.max(0, Math.round(o.km || 0)), date: o.date || todayStr(), remplaceId: oldF.id, groupe: oldF.groupe || null };
  if (o.adjustInterval) { const observed = newF.kmDebut - (oldF.kmDebut || 0); if (observed > 0) newF.kmPrevus = observed; } // intervalle réel observé → remet du réel dans la provision
  oldF.statut = 'remplace'; oldF.remplaceParId = newF.id; oldF.remplaceAt = newF.date;
  const idx = S.frais.findIndex((x) => x.id === oldF.id); if (idx >= 0) S.frais.splice(idx + 1, 0, newF); else S.frais.push(newF);
  return newF;
}
// 🧾 Note de frais véhicule (flux central) : remplacement d'un poste suivi (+ éléments liés impactés) / nouveau poste à suivre / dépense ponctuelle. Toujours journalisée (réel).
function modalNoteFrais(opts) {
  opts = opts || {};
  const suivis = () => (S.frais || []).filter((f) => f.statut !== 'remplace');
  const childrenOf = (id) => (S.frais || []).filter((x) => x.parentId === id && x.statut !== 'remplace');
  const first = suivis()[0];
  const st = { mode: 'remplace', cibleId: opts.preselectId || (first ? first.id : ''), date: todayStr(), km: Math.round(odometer()), montant: 0,
    poste: '', nvNature: 'recurrent', nvKmPrevus: 0, childChoice: {}, adjInterval: false, adjMontant: false };
  const render = () => {
    const list = suivis();
    const cible = list.find((f) => f.id === st.cibleId) || null;
    const kids = (st.mode === 'remplace' && cible && !cible.parentId) ? childrenOf(cible.id) : [];
    const kidsChanged = kids.some((k) => st.childChoice[k.id] === 'change');
    let sugg = '';
    if (st.mode === 'remplace' && cible) {
      const observed = st.km - (cible.kmDebut || 0);
      sugg = `<label class="chk2"><input type="checkbox" id="nfAdjI" ${st.adjInterval ? 'checked' : ''}/> Ajuster l'intervalle du nouveau poste au réel observé (${observed > 0 ? km(observed) : '—'})</label>`;
      if (!kids.length || !kidsChanged) sugg += `<label class="chk2"><input type="checkbox" id="nfAdjM" ${st.adjMontant ? 'checked' : ''}/> Aligner la provision sur le montant réel (${eur(st.montant)})</label>`;
    }
    openModal(`<div class="modal-head"><b>🧾 Note de frais véhicule</b><button class="x" id="mX">✕</button></div>
      <div class="row"><label class="grow">Date<input type="date" id="nfDate" value="${st.date}" max="${todayStr()}"/></label><label class="grow">Km compteur<input type="number" id="nfKm" step="1" min="0" inputmode="numeric" value="${st.km}"/></label></div>
      <label>Montant réel HT<input type="number" id="nfMontant" step="1" min="0" inputmode="numeric" value="${st.montant || ''}"/></label>
      <p class="hint">Cette dépense correspond à…</p>
      <label class="chk2"><input type="radio" name="nfMode" value="remplace" ${st.mode === 'remplace' ? 'checked' : ''}/> le remplacement d'un poste suivi</label>
      <label class="chk2"><input type="radio" name="nfMode" value="nouveau" ${st.mode === 'nouveau' ? 'checked' : ''}/> un nouveau poste récurrent à suivre</label>
      <label class="chk2"><input type="radio" name="nfMode" value="ponctuel" ${st.mode === 'ponctuel' ? 'checked' : ''}/> une dépense ponctuelle (sans suivi)</label>
      ${st.mode === 'remplace' ? (list.length ? `<label>Poste remplacé<select id="nfCible">${list.map((f) => `<option value="${f.id}"${f.id === st.cibleId ? ' selected' : ''}>${f.parentId ? '↳ ' : ''}${esc(f.poste || 'Frais')}</option>`).join('')}</select></label>
        ${kids.length ? `<p class="hint">Éléments liés — sont-ils impactés ?</p>${kids.map((k) => `<div class="row" style="align-items:center;gap:8px"><span class="grow">↳ ${esc(k.poste || 'Élément')}</span><select data-kc="${k.id}"><option value="maintien"${(st.childChoice[k.id] || 'maintien') === 'maintien' ? ' selected' : ''}>Maintenu</option><option value="change"${st.childChoice[k.id] === 'change' ? ' selected' : ''}>Changé aussi</option><option value="abandon"${st.childChoice[k.id] === 'abandon' ? ' selected' : ''}>Abandonné</option></select></div>`).join('')}` : ''}
        ${sugg}` : '<p class="empty">Aucun poste suivi à remplacer.</p>') : ''}
      ${st.mode === 'nouveau' ? `<label>Libellé du poste<input type="text" id="nfPoste" value="${esc(st.poste)}" placeholder="Ex. Courroie de distribution"/></label>
        <div class="row"><label class="grow">Nature<select id="nfNature"><option value="recurrent"${st.nvNature === 'recurrent' ? ' selected' : ''}>Récurrent</option><option value="exceptionnel"${st.nvNature === 'exceptionnel' ? ' selected' : ''}>Exceptionnel</option></select></label><label class="grow">Intervalle (km)<input type="number" id="nfKmPrevus" step="1000" min="0" value="${st.nvKmPrevus || ''}"/></label></div>` : ''}
      ${st.mode === 'ponctuel' ? `<label>Libellé<input type="text" id="nfPoste" value="${esc(st.poste)}" placeholder="Ex. Lavage, petite réparation…"/></label>` : ''}
      <div class="actions"><button class="btn primary block" id="nfOk">Enregistrer</button></div>`);
    $('mX').onclick = closeModal;
    const sync = () => { st.date = $('nfDate').value || todayStr(); st.km = Math.max(0, Math.round(parseNum($('nfKm').value) || 0)); st.montant = Math.max(0, parseNum($('nfMontant').value) || 0); if ($('nfPoste')) st.poste = $('nfPoste').value; if ($('nfNature')) st.nvNature = $('nfNature').value; if ($('nfKmPrevus')) st.nvKmPrevus = Math.max(0, Math.round(parseNum($('nfKmPrevus').value) || 0)); };
    ['nfDate', 'nfKm', 'nfMontant'].forEach((id) => { const e = $(id); if (e) e.addEventListener('input', sync); });
    document.querySelectorAll('[name="nfMode"]').forEach((rb) => rb.addEventListener('change', (e) => { sync(); st.mode = e.target.value; render(); }));
    if ($('nfCible')) $('nfCible').addEventListener('change', (e) => { sync(); st.cibleId = e.target.value; st.childChoice = {}; render(); });
    document.querySelectorAll('[data-kc]').forEach((s) => s.addEventListener('change', (e) => { sync(); st.childChoice[e.target.dataset.kc] = e.target.value; render(); }));
    if ($('nfAdjI')) $('nfAdjI').addEventListener('change', (e) => { st.adjInterval = e.target.checked; });
    if ($('nfAdjM')) $('nfAdjM').addEventListener('change', (e) => { st.adjMontant = e.target.checked; });
    $('nfOk').onclick = () => {
      sync();
      if (st.date > todayStr()) { alert('La date ne peut pas être dans le futur.'); return; }
      if (st.mode === 'remplace') {
        const cible = list.find((f) => f.id === st.cibleId); if (!cible) { alert('Choisissez le poste remplacé.'); return; }
        const newF = replaceFrais(cible, { km: st.km, date: st.date, montantHT: st.adjMontant ? st.montant : null, adjustInterval: st.adjInterval });
        kids.forEach((k) => { const c = st.childChoice[k.id] || 'maintien';
          if (c === 'change') replaceFrais(k, { km: st.km, date: st.date, adjustInterval: st.adjInterval, parentId: newF.id }); // élément changé → nouvel élément sous le nouveau type
          else if (c === 'abandon') { k.statut = 'remplace'; k.remplaceAt = st.date; } // abandonné → archivé sous l'ancien type
          else { k.parentId = newF.id; } // maintenu → rattaché au nouveau type (garde sa propre échéance)
        });
        pushFraisJournal(st.date, st.km, fraisTypeHeadId(newF.id), newF.poste, st.montant);
      } else if (st.mode === 'nouveau') {
        if (!st.poste) { alert('Donnez un libellé au poste.'); return; }
        const nf = { id: uid(), poste: st.poste, nature: st.nvNature, statut: 'actif', kmReport: 0, montantHT: st.montant, kmPrevus: st.nvKmPrevus, kmDebut: st.km, date: st.date, parentId: null };
        S.frais.push(nf); pushFraisJournal(st.date, st.km, nf.id, nf.poste, st.montant);
      } else { pushFraisJournal(st.date, st.km, null, st.poste || 'Dépense ponctuelle', st.montant); }
      saveSettings(); closeModal(); refreshFraisUI(); alert('Note de frais enregistrée.');
    };
  };
  render();
}
...
function renderPlancheConfig() {
  ...
  const gg = S.planche.gridGuide || (S.planche.gridGuide = { marginLR: 0.5, marginTB: 0.5 });
...
function plRenderDateSuggest() {
  ...
  const nn = norm(plCreate && plCreate.cheval); const sug = [], seen = new Set();
  if (nn) (S.plancheTodo || []).forEach((y) => { if (norm(y.chevalNom) !== nn) return; if (hasPlancheDone(y.clientId, y.chevalNom, y.date, y.type)) return; const k = (y.date || '') + '|' + norm(y.type); if (seen.has(k)) return; seen.add(k); sug.push({ date: y.date || '', type: y.type || '' }); });
  sug.sort((a, b) => (b.date || '').localeCompare(a.date || '') || (a.type || '').localeCompare(b.type || ''));
...
function modalPlancheCreate(type, prefill) {
  ...
  openModal(`<div class="modal-head"><b>🖼 ${titre}</b><button class="x" id="mX">✕</button></div>
    <div style="max-height:80vh;overflow:auto" id="plCbody">
      ...
      <section class="card">
        ...
        <div id="plCgridEditCtrls" class="pl-edit-ctrls" style="display:none"><button class="btn small" id="plCgridCropMode">✂️ Rogner des images</button><button class="btn small primary" id="plCgridCropOk" style="display:none">✅ Valider le rognage (<span id="plCropN">0</span>)</button><button class="btn small" id="plCgridCropCancel" style="display:none">Annuler</button></div>
        <p class="hint" id="plCgridEditHint" style="display:none">Mode édition : touchez une image pour la <b>cadrer</b> (zoom, déplacement, rotation). Le cadrillage (repères) n'apparaît pas sur le PDF. <b>Rogner des images</b> : activez, touchez les images à rogner (cadre rouge), puis <b>Valider</b> — elles seront agrandies pour que le 1ᵉʳ repère remplisse la case. Rebasculez pour revenir au mode normal (toucher une image = la retirer).</p>
        <div id="plCgrid"></div>
      </section>
  ...
  $('plCbody').querySelectorAll('#plCmod .seg-btn').forEach((b) => b.addEventListener('click', () => {
    plCreate.modele = b.dataset.plcm; plCreate.angles = (P.modeles[plCreate.modele] || []).slice(); plCreate.cells = {}; plCreate.cellT = {}; plCreate.cellCrop = {}; plCreate.sel = null;
  ...
  if ($('plCgridCropMode')) { // « Rogner des images » : sélection par image (cadre rouge) + Valider
    $('plCgridCropMode').addEventListener('click', () => { plCreate.cropMode = !plCreate.cropMode; plCreate.cropSel = plCreate.cropMode ? new Set(Object.keys(plCreate.cellCrop || {})) : null; plUpdateCropCtrls(); plRenderGrid(); });
    $('plCgridCropOk').addEventListener('click', () => { const nc = {}; (plCreate.cropSel || new Set()).forEach((k) => { if (plCreate.cells[k]) nc[k] = true; }); plCreate.cellCrop = nc; plCreate.cropMode = false; plCreate.cropSel = null; plUpdateCropCtrls(); plRenderGrid(); });
    $('plCgridCropCancel').addEventListener('click', () => { plCreate.cropMode = false; plCreate.cropSel = null; plUpdateCropCtrls(); plRenderGrid(); });
  }
  if ($('plCgridEdit')) $('plCgridEdit').addEventListener('click', () => {
    plCreate.gridEdit = !plCreate.gridEdit;
    if (!plCreate.gridEdit) { plCreate.cropMode = false; plCreate.cropSel = null; } // sortie d'édition → quitte aussi le mode rognage
...
function plPlace(key) {
  ...
  Object.keys(plCreate.cells).forEach((k) => { if (plCreate.cells[k] === plCreate.sel) { delete plCreate.cells[k]; delete plCreate.cellT[k]; delete plCreate.cellCrop[k]; } }); // une photo = une seule case (elle déménage → sa transfo/rognage tombe)
  delete plCreate.cellT[key]; delete plCreate.cellCrop[key]; // nouvelle photo dans la case → repart neuf
  ...
  pages.forEach((pg, pi) => {
    ...
    if (st.type === 'avantapres' && pi === 0) {
      ...
      (st.compar || []).forEach((c, ci2) => {
        ...
        if (x) x.addEventListener('click', () => {
          ...
          st.cells = remap(st.cells); if (st.cellT) st.cellT = remap(st.cellT); if (st.cellCrop) st.cellCrop = remap(st.cellCrop);
          plRenderPot(); plRenderGrid();
        });
    ...
    tbl.querySelectorAll('.pl-cell').forEach((td) => {
      ...
      td.addEventListener('click', () => {
        ...
        if (st.cells[key]) { delete st.cells[key]; if (st.cellT) delete st.cellT[key]; if (st.cellCrop) delete st.cellCrop[key]; plRenderPot(); plRenderGrid(); } else { plPlace(key); }
      });
...
// Met à jour les boutons de rognage (libellé du bouton mode, visibilité de Valider/Annuler, compteur).
function plUpdateCropCtrls() {
  const st = plCreate, md = $('plCgridCropMode'); if (!st || !md) return;
  const on = st.cropMode, n = st.cropSel ? st.cropSel.size : 0;
  md.classList.toggle('primary', on);
  md.textContent = on ? '✂️ Touchez les images à rogner' : '✂️ Rogner des images';
  const okb = $('plCgridCropOk'); if (okb) okb.style.display = (on && n >= 1) ? '' : 'none';
...
// ===== Compte rendu photo (planches à faire) — clé PAR TYPE (stade) : client|cheval|date|type. Legacy (sans type) = un seul bucket ''. =====
const plancheTodoKey = (clientId, chevalNom, date, type) => clientId + '|' + norm(chevalNom || '') + '|' + (date || '') + '|' + norm(type || '');
function addPlancheTodo(x) {
  if (!Array.isArray(S.plancheTodo)) S.plancheTodo = [];
  const k = plancheTodoKey(x.clientId, x.chevalNom, x.date, x.type);
  if (S.plancheTodo.some((y) => plancheTodoKey(y.clientId, y.chevalNom, y.date, y.type) === k)) return false;
  S.plancheTodo.push(Object.assign({ id: uid() }, x)); saveSettings(); return true;
}
const hasPlancheTodo = (clientId, chevalNom, date, type) => (S.plancheTodo || []).some((y) => plancheTodoKey(y.clientId, y.chevalNom, y.date, y.type) === plancheTodoKey(clientId, chevalNom, date, type));
function removePlancheTodo(clientId, chevalNom, date, type) { const k = plancheTodoKey(clientId, chevalNom, date, type); S.plancheTodo = (S.plancheTodo || []).filter((y) => plancheTodoKey(y.clientId, y.chevalNom, y.date, y.type) !== k); saveSettings(); }
...
function syncPhotoTodos(clientId, cv, date, tourId) {
  ...
  (((cv.photo && cv.photo.stades) || [])).forEach((sd) => addPlancheTodo({ clientId, chevalId: cv.id, chevalNom: cv.nom, date, tourId, type: sd, angle: (cv.photo || {}).angle }));
}
// État planche d'un CLIENT à une date, par (cheval × stade) : 'none' | 'red' (aucune faite) | 'orange' (partiel) | 'green' (toutes faites).
function plancheStateForClient(t, clientId) {
  const date = t.date; let total = 0, done = 0;
  (t.arrets || []).forEach((a) => (a.clients || []).forEach((cl) => { if (cl.clientId !== clientId) return; (cl.chevaux || []).forEach((cv) => {
    if (chevalCancelled(cv)) return;
...
// Acte « Photo » d'un cheval : choix du tarif (angles 3/4/5) + type(s) de planche (stades, multi). Enregistre cv.photo = { angle, stades:[] } ; radio suivie mais non facturée.
function modalPhotoPick(cv, onDone) {
  const cur = cv.photo || {}; let angle = cur.angle || 3; const sel = new Set(cur.stades || []);
  const stades = S.planche.stades || [];
  openModal(`<div class="modal-head"><b>📷 Photos — ${esc(cv.nom)}</b><button class="x" id="mX">✕</button></div>
    <p class="hint">Choisissez le <b>tarif</b> (nombre d'angles) et le(s) <b>type(s) de planche</b>. Plusieurs types possibles pour un même cheval. La <b>radio</b> est suivie mais <b>non facturée</b>.</p>
    <label>Tarif (angles)</label>
    <div class="seg" id="phAngles" style="flex-wrap:wrap"></div>
    <p class="hint" id="phTarif" style="margin:6px 0"></p>
    <label>Type(s) de planche</label>
    <div id="phStades"></div>
    <div class="actions two"><button class="btn" id="phCancel">Annuler</button><button class="btn primary" id="phOk">Enregistrer</button></div>`);
  const seg = $('phAngles'), tarifEl = $('phTarif'), box = $('phStades');
  const paintTarif = () => { const n = [...sel].filter(plancheStadeBillable).length; tarifEl.innerHTML = n ? `${n} planche(s) facturable(s) × ${eur(ttc(photoTariffHT(angle)))} = <b>${eur(ttc(photoTariffHT(angle) * n))} TTC</b>` : '<i>Aucune planche facturable (radio seule ou aucun type).</i>'; };
  const paintAngles = () => { seg.innerHTML = [3, 4, 5].map((a) => `<button type="button" class="seg-btn${a === angle ? ' on' : ''}" data-pa="${a}">${a} angles · ${eur(ttc(photoTariffHT(a)))}</button>`).join(''); seg.querySelectorAll('[data-pa]').forEach((b) => b.addEventListener('click', () => { angle = +b.dataset.pa; paintAngles(); paintTarif(); })); };
  paintAngles();
  stades.forEach((sd) => { const row = document.createElement('label'); row.className = 'chk2'; row.innerHTML = `<input type="checkbox" ${sel.has(sd) ? 'checked' : ''}/> ${esc(sd)}${plancheStadeBillable(sd) ? '' : ' <span class="li-sub">— non facturée</span>'}`; row.querySelector('input').addEventListener('change', (e) => { if (e.target.checked) sel.add(sd); else sel.delete(sd); paintTarif(); }); box.appendChild(row); });
  paintTarif();
  const finish = (save) => { closeModal(); if (save) { if (sel.size) cv.photo = { angle, stades: stades.filter((sd) => sel.has(sd)) }; else delete cv.photo; } if (onDone) onDone(); };
  $('mX').onclick = () => finish(false); $('phCancel').onclick = () => finish(false); $('phOk').onclick = () => finish(true);
}
...
function modalArretPlanche(t, a, onlyClientId) {
  ...
  rows.forEach(({ cl, cv }) => {
    const stades = (cv.photo && cv.photo.stades) || [];
    let cvSt = 'none';
    if (stades.length) { const dn = stades.filter((sd) => hasPlancheDone(cl.clientId, cv.nom, t.date, sd)).length; cvSt = dn === 0 ? 'red' : dn < stades.length ? 'orange' : 'green'; }
...
function renderComptePhoto() {
  ...
  items.forEach((x) => {
    const el = document.createElement('div'); el.className = 'list-item stack-act';
    el.innerHTML = `<div class="li-main"><b>🐴 ${esc(x.chevalNom)}</b>${x.type ? ' <span class="td-eta td-past">' + esc(x.type) + '</span>' : ''}<span class="li-sub">${esc(clientName(x.clientId))} · RDV du ${esc(fmtDateFr(x.date))}</span></div><div class="li-act li-act-col"><button class="btn small primary" data-make>🖼 Créer la planche</button><button class="btn small" data-rm>Retirer</button></div>`;
    el.querySelector('[data-make]').addEventListener('click', () => modalPlancheCreate('contact', { cheval: x.chevalNom, client: clientName(x.clientId), clientId: x.clientId, date: x.date, stade: x.type || '', todoId: x.id }));
...
function renderHomeTrajet() {
  ...
  todays.forEach((t) => {
    ...
    // ----- Retour → domicile/arrivée : « Agir » (Waze + Route retour) + « Clôturer tournée » (inactif tant que non démarrée) -----
    {
      ...
      const retRDone = realRet != null && realRet > 0; const retRStale = realRet === 0;
      const retLbl = (estRet != null ? durMin(estRet) + ' est.' : '—') + (retRDone ? ' · <b>' + durMin(realRet) + ' réel</b>' : (retRStale ? ' · <b class="td-stale">🔄 à recalculer</b>' : '')) + (t.endedAt ? ' · ✅ ' + hm(t.endedAt) : '');
...
function modalRouteTime(tour, arret, estMin, after) {
  ...
  $('rtOk').addEventListener('click', () => { const v = parseInt($('rtMin').value, 10); if (isNaN(v) || v <= 0) delete arret.realMin; else arret.realMin = v; persist(); closeModal(); (after || renderHomeTrajet)(); }); // 0 (ou vide) = effacer ; le 0 est réservé au marqueur « à recalculer »
...
function modalReturnTime(t, estMin, after) {
  ...
  const rtConv = () => { const v = parseInt($('rtMin').value, 10); $('rtConv').innerHTML = (!isNaN(v) && v >= 0) ? '= <b>' + durMin(v) + '</b>' : ''; };
  $('rtMin').addEventListener('input', rtConv); rtConv();
  $('rtOk').addEventListener('click', () => { const v = parseInt($('rtMin').value, 10); if (isNaN(v) || v <= 0) delete t.returnRealMin; else t.returnRealMin = v; persist(); closeModal(); (after || renderHomeTrajet)(); }); // 0 (ou vide) = effacer ; le 0 est réservé au marqueur « à recalculer »
...
function modalCancelRdv(nom, opts) {
  ...
  openModal(`<div class="modal-head"><b>Annuler / reporter — ${esc(nom)}</b><button class="x" id="mX">✕</button></div>
    ...
    <div id="cxReportBox" style="display:none"><label>Montant TTC à facturer à la prochaine visite (facultatif)<input type="number" id="cxMontant" step="0.01" min="0" inputmode="decimal"/></label><p class="hint">Mémorisé et ajouté automatiquement à la prochaine tournée de ce client.</p></div>
    <p class="hint" id="cxNote2"></p>
...
// RDV à venir d'un cheval, à moins de 5 semaines d'une date donnée → alerte « déjà un RDV » (informative).
// fromDate = date de référence (tournée ouverte) : on ne regarde qu'à partir du LENDEMAIN (le RDV du jour de la tournée est ignoré).
// excludeTourId = la tournée d'où l'on prend le RDV (à ne jamais compter comme doublon).
function chevalRdvConflicts(clientId, chevalId, newDate, fromDate, excludeTourId) {
  const start = addDaysStr(fromDate || todayStr(), 1);
  const p = (s) => { const [y, m, d] = s.split('-').map(Number); return Date.UTC(y, (m || 1) - 1, d || 1); };
  const out = [];
  allTours().forEach((t) => {
    if (excludeTourId && t.id === excludeTourId) return; // la tournée ouverte elle-même n'est pas un doublon
    if (statusOf(t) === 'cloturee' || !t.date || t.date < start) return;
...
function chevalRdvWarnHtml(clientId, chevalId, newDate, fromDate, excludeTourId) {
  const c = chevalRdvConflicts(clientId, chevalId, newDate, fromDate, excludeTourId);
  ...
  const render = () => {
    ...
    entries.forEach((en) => {
      ...
      if (en.hasPatho) {
        ...
        if (en.pathoStatus === 'maintenir') inner += `<div class="rdv-dh"><label>Date du 2ᵉ RDV<input type="date" data-psdate value="${en.pathoDate}"/></label><label>Heure<input type="time" data-psheure value="${en.pathoHeure}"/></label></div><p class="hint" data-psprev></p>` + chevalRdvWarnHtml(client.id, en.id, en.pathoDate, t.date, t.id);
        inner += `</div>`;
      }
      wrap.innerHTML = inner;
    ...
    $('rdvOk').addEventListener('click', () => {
      ...
      if (!hasParage && !hasPathoMaint) { alert('📅 Sélectionnez au moins un cheval avec une date et une heure pour programmer un RDV.'); return; }
...
function fraisEchuItemEl(f) {
  const el = document.createElement('div'); el.className = 'list-item stack-act';
  const seuil = fraisSeuil(f), odo = Math.round(odometer()), dep = Math.max(0, odo - seuil);
  const rep = (f.kmReport || 0) ? ' · report +' + km(f.kmReport) : '';
  el.innerHTML = `<div class="li-main"><b>🧾 ${esc(f.poste || 'Frais')}${f.parentId ? ' <span class="li-sub">(élément)</span>' : ''}</b><span class="li-sub">⚠ échu · ${km(odo)} / ${km(seuil)}${dep ? ' (dépassement ' + km(dep) + ')' : ''}${rep}</span></div><div class="li-act li-act-col"><button class="btn small primary" data-chg>✅ Changement fait</button><button class="btn small" data-rep>⏳ Repousser</button></div>`;
  el.querySelector('[data-chg]').addEventListener('click', () => modalNoteFrais({ preselectId: f.id }));
  el.querySelector('[data-rep]').addEventListener('click', () => modalRepousser(f));
  return el;
}
// Section Accueil « Statut véhicule » : visible si relevé du mois dû (rappel mensuel) OU frais échus à contrôler.
function renderVehiculeStatut() {
  ...
  if (hint) { const last = lastOdoReleve();
    ...
    else hint.innerHTML = echus.length ? `${echus.length} poste(s) véhicule échu(s) à contrôler ci-dessous.` : `Relevé du mois enregistré ✔ — estimé actuel ${km(odometer())}.`;
  }
  ...
  const box = $('homeFraisEchus'); if (box) { box.innerHTML = '';
    echus.slice(0, 2).forEach((f) => box.appendChild(fraisEchuItemEl(f))); // 2 aperçus ; le reste via « + N » → page complète
    if (echus.length > 2) {
      const more = document.createElement('div'); more.className = 'list-item clickable';
      more.innerHTML = `<div class="li-main"><b>+ ${echus.length - 2} autre(s) échu(s)</b><span class="li-sub">Voir la liste complète (Gestion → Statut véhicule)</span></div><div class="li-act"><span class="li-chev">›</span></div>`;
...
function renderStatutVehiculePage() {
  ...
  const bnote = $('btnNoteFrais'); if (bnote) bnote.onclick = () => modalNoteFrais();
  // Contrôle véhicule (échus) : listés ici aussi, même après avoir quitté l'Accueil.
  const fbox = $('statutFraisEchus'); if (fbox) {
    ...
    echus.forEach((f) => fbox.appendChild(fraisEchuItemEl(f)));
  }
  // Historique des remplacements (postes/éléments inactivés).
  const rbox = $('statutRemplaces'); if (rbox) {
    rbox.innerHTML = ''; const rempl = (S.frais || []).filter((f) => f.statut === 'remplace').sort((a, b) => (b.remplaceAt || b.date || '').localeCompare(a.remplaceAt || a.date || ''));
    if ($('statutRemplacesEmpty')) $('statutRemplacesEmpty').style.display = rempl.length ? 'none' : 'block';
    rempl.forEach((f) => {
      const el = document.createElement('div'); el.className = 'list-item';
      el.innerHTML = `<div class="li-main"><b>🧾 ${esc(f.poste || 'Frais')}${f.parentId ? ' <span class="li-sub">(élément)</span>' : ''}</b><span class="li-sub">remplacé${f.remplaceAt ? ' le ' + esc(fmtDateFr(f.remplaceAt)) : ''} · départ ${km(f.kmDebut || 0)}</span></div>`;
      rbox.appendChild(el);
    });
  }
}
...
function fraisEchus() {
  const odo = odometer();
  return (S.frais || []).filter((f) => f.statut !== 'remplace' && (f.kmPrevus > 0) && odo >= fraisSeuil(f));
}
